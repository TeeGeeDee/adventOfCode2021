...
#= Ideas:
    Ultimately (unless already in finished position) each amphipod moves to a space in the hallwa and then into the final room
...
const HALLWAY        = [PositionA((x,2)) for x in HALLWAY_X];
struct Amphipod
    type::AmphipodType
...
const ExtraToPaths = Dict{Tuple{Amphipod,Int},Vector{PositionA}};
...
const StartSummary = NamedTuple{(:gamestate,:gamestatefull,:amphipods,:positions,:isdone,:types,:baselinecost,:roomsize,:allmoves),
                                Tuple{
                                    GameState,
                                    GameStateFull,
                                    Vector{Amphipod},
                                    ...
                                    Int,
                                    Int,
                                    OrderedDict{Amphipod,Vector{Pair{PositionA,Int}}}}};
...
function getallmoves(positions::GameState)::OrderedDict{Amphipod,Vector{Pair{PositionA,Int}}}
    return OrderedDict{Amphipod,Vector{Pair{PositionA,Int}}}(a=>[Pair(hallpos,begin
                                                                tofromx = sort([pos[1],SIDE_ROOM_X[a.type]]);
                                                                if     all(hallpos[1].<tofromx) cost = 2*(tofromx[1]-hallpos[1]);
                                                                elseif all(hallpos[1].>tofromx) cost = 2*(hallpos[1]-tofromx[2]);
                                                                else                            cost = 0;
                                                                end
                                                                cost * AMPHIPOD_COSTS[a.type];
                                                                end) for hallpos in HALLWAY] for (a,pos) in positions);
end
...
function getmincost(startsummary::StartSummary)::Int
    ...
    cost = 0;
    queue = PriorityQueue{GameStateFull,Int}();
...
function validmoves(gamestatefull::GameStateFull,startsummary::StartSummary)::Vector{Tuple{Amphipod,PositionA,Int}}
    ...
    out = Vector{Tuple{Amphipod,PositionA,Int}}();
    for type in instances(AmphipodType)
